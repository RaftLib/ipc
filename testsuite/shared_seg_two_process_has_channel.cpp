                ...
                ipc::buffer *buffer ) 
{
    ...
    /**
     * let's wait until the consumer produces the shared segment. 
     */
    ipc::buffer::has_channel( tls_producer, channel_id + 1, true /** blocking **/ );
    
    /** add segment to local TLS **/
    if( ipc::buffer::add_shared_segment( tls_producer,
                                         channel_id + 1 /** seg channel **/,
                                         sizeof( shm_seg )
                                         /** no init function, consumer must do that **/ ) == ipc::channel_err )
    {
        std::cout << "channel error in producer\n";
        assert( false );
    }
    ...
    if( shared_seg->value != -1 )
    {   
        std::cout << "shared segment value should be initialized to (-1), exiting\n";
        exit( EXIT_FAILURE );
    }   
                ...
                ipc::buffer *buffer ) 
{
    ...
    /** add wait so producer has to wait on channel add **/
    sleep( 1 ); 
